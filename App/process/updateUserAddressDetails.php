...
if (isset($_POST["addressNo"]) && isset($_POST["addressLine1"]) && isset($_POST["addressLine2"])) {
    ...
    if (!empty($address_no) && !empty($address_line1) && !empty($address_line2)) {
        ...
        if ($user_has_address_result->num_rows > 0) {
            Database::execute("UPDATE user_has_address SET `no` = '$address_no', `line1` = '$address_line1', `line2` = '$address_line2' WHERE `user_email` = '$email'");
        } else {
            Database::execute("INSERT INTO user_has_address (`user_email`, `no`, `line1`, `line2`) VALUES ('$email', '$address_no', '$address_line1', '$address_line2')");
        }
