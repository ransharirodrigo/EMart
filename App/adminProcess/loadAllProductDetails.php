...
model.model_name AS product_model_name,
product_images.path AS image_path FROM emart_db.product
...
INNER JOIN model ON model.model_id=product.model_model_id LEFT JOIN emart_db.product_images ON product_images.product_product_id=product.product_id ORDER BY `product`.`product_id` 
 ASC");
...
?>
        ...
        <tr onclick="productViewPopUp(
            ...
            '<?php echo $product_data['product_model_id'] ?>',
                  ...
                  '<?php echo $product_data['image_path'] ?>'
            )" class="tableRow">
